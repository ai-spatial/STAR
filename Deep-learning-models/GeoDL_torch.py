...
# @License: MIT License
# PyTorch version of GeoDL — same API, uses models_torch.
...
from config import *
from models_torch import DNNmodel, UNetmodel, LSTMmodel
...
class GeoDL():
  """
  Geo-aware deep learning model with STAR partitioning (PyTorch backend).
  Same API as GeoDL in GeoDL.py; only the model implementation is PyTorch.
  """
  def __init__(self,
  ...
  def _build_model(self):
    ...
    elif self.model_choice.lower() in ['lstm', 'long short-term memory']:
      kwargs = dict(self.model_kwargs)
      kwargs.setdefault('n_time', N_TIME)
      return LSTMmodel(ckpt_path=self.dir_ckpt, **kwargs)
  ...
  def visualize_grid(self, Xtest, ytest, Xtest_group, step_size=STEP_SIZE):
    if self.s_branch is None:
      raise ValueError("Model not trained. Call fit() first.")
    try:
      from models_torch import predict_test_group_wise
    except ImportError:
      from models import predict_test_group_wise
