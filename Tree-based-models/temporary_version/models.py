...
'''This is an example implementation for LSTM. Can be used as a template.'''
class LSTMmodel():#tf.keras.Model

  def __init__(self, path, #ckpt_path,
               layer_size = LAYER_SIZE, num_layers = NUM_LAYERS, num_class = NUM_CLASS,
               mode = MODE, name = None,
    ...
    #this attribute was added to RF and used in functions (e.g., transformation)
    self.name = 'lstm'
  ...
  def create_net(self):
    ...
    dnn_output_size = 10
    net_layers = [#same number as num_layers
      ...
      tf.keras.layers.Dense(16, activation=tf.nn.relu, kernel_initializer=initializer),
      tf.keras.layers.Dense(10, activation=tf.nn.relu, kernel_initializer=initializer),
      tf.keras.layers.Dense(10, activation=tf.nn.relu, kernel_initializer=initializer),
      # tf.keras.layers.Dense(10, activation=tf.nn.relu, kernel_initializer=initializer),
      tf.keras.layers.Dense(dnn_output_size, activation=tf.nn.relu, kernel_initializer=initializer)
    ]

    lstm = tf.keras.layers.LSTM(128, activation='tanh', kernel_initializer=initializer, return_sequences=False)
    # lstm_att = tf.keras.layers.LSTM(layer_size, activation='sigmoid', kernel_initializer=initializer, return_sequences=False)
    # inputs = tf.keras.Input(shape=(INPUT_SIZE,))
    # x = inputs
    # x = tf.reshape(inputs[:, 0:330],(-1,N_TIME_FEATURE))
    # x_other = inputs[:, 330:333]
    inputs = tf.keras.Input(shape=(N_TIME, (N_TIME_FEATURE + N_OTHER_FEATURE)) )
    x = tf.reshape(inputs,(-1, (N_TIME_FEATURE + N_OTHER_FEATURE)) )
    for i in range(len(net_layers)):#self.num_layers
      # x = tf.keras.layers.Dense(self.layer_size, activation=tf.nn.relu, kernel_initializer=initializer)(x)
      x = net_layers[i](x)

    x = tf.reshape(x, (-1, N_TIME, dnn_output_size))
    x = lstm(x)
  ...
  def save(self, branch_id):
    ...
    self.model.save_weights(self.path + '/' + self.model_name + '_ckpt_' + branch_id)
  ...
  def predict_test(self, X, y, X_group, s_branch, prf = True, X_branch_id = None):
...
class DNNmodel():#tf.keras.Model
  ...
  def create_net(self):
    ...
    net_layers = [#same number as num_layers
      ...
      tf.keras.layers.Dense(16, activation=tf.nn.relu, kernel_initializer=initializer),
      tf.keras.layers.Dense(16, activation=tf.nn.relu, kernel_initializer=initializer),
      tf.keras.layers.Dense(8, activation=tf.nn.relu, kernel_initializer=initializer),
      tf.keras.layers.Dense(8, activation=tf.nn.relu, kernel_initializer=initializer)
    ]
